...
import { Sparkles, ArrowUpCircle, Camera, Wand2 } from 'lucide-react';
...
const ScreenResults: React.FC<Props> = ({ result, imagePreview, onReset }) => {
  ...
  const goodGarments = garments.filter(g => 
    !verdict.primary_issue.toLowerCase().includes(g.garment.toLowerCase()) && g.confidence >= 0.15
  );
  const goodGarmentNames = goodGarments.map(g => g.garment).join(" + ");
